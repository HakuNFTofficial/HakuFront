    ...
    OwnableUpgradeable
{
    ...
    /// @notice 提取原生币（Arc native USDC）到 owner
    ...
    /// @param currency0 池子的 currency0 地址（Arc native USDC 使用 address(0)）
    /// @param currency1 池子的 currency1 地址（HakuToken 地址）
    /// @param fee 池子的手续费率（从 PoolConfig.FEE 读取，例如 3002 表示 0.3002%）
    /// @param tickSpacing tick spacing（从 PoolConfig.TICK_SPACING 读取，如 59 或 60）
    /// @param hooks hooks 地址（从 PoolConfig.HOOKS 读取）
    /// @param tickLower position 的下边界 tick（从 PoolConfig.TICK_LOWER 读取，如 -887183）
    /// @param tickUpper position 的上边界 tick（从 PoolConfig.TICK_UPPER 读取，如 887183）
    /// @param liquidityAmount 要移除的流动性数量（正数，函数内部会转为负数）
    /// @param salt position 的 salt（从 PoolConfig.SALT 读取）
    ...
    /// @notice 添加流动性（可多次调用）
    /// @param amountA Amount of tokenA (Arc native USDC) to add
    ...
    function addLiquidity(uint256 amountA) external payable {
        ...
        // tokenA is always address(0) (Arc native USDC) from PoolConfig
        require(msg.value == amountA, "Incorrect native USDC amount");
        require(
    ...
    function unlockCallback(
        ...
        if (
            ...
            // -------- 4. 组装参数 --------
            // 使用接近全范围的流动性（从 PoolConfig 读取 TICK_LOWER 到 TICK_UPPER）
            // 这样可以覆盖 MIN_TICK 到 MAX_TICK，避免价格超出范围
            // tick 必须与 tickSpacing 对齐
            // MIN_TICK = -887272, MAX_TICK = 887272
            // tickSpacing = 59: floor(-887272/59)*59 = -887183, floor(887272/59)*59 = 887183
            // tickSpacing = 60: floor(-887272/60)*60 = -887220, floor(887272/60)*60 = 887220
            //
            ...
            IPoolManager.ModifyLiquidityParams memory params = IPoolManager
                .ModifyLiquidityParams({
                    tickLower: PoolConfig.TICK_LOWER, // 从 PoolConfig 读取，对齐到 tickSpacing
                    tickUpper: PoolConfig.TICK_UPPER, // 从 PoolConfig 读取，对齐到 tickSpacing
                    // forge-lint: disable-next-line(unsafe-typecast)
