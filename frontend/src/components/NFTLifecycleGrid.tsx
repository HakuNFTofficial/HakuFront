...
type ProfileFilter = 'profile' | 'mintable' | 'minting' | 'burnable'
...
const EMPTY_COUNTS: NftStatusCounts = {
    in_progress: 0,
    mintable: 0,
    minting: 0,
    burnable: 0,
...
function getLifecycleStatus(nft: NFT) {
    if (nft.is_mint === 2) {
        return { label: 'Burnable', className: 'border-red-500/50 bg-red-500/20 text-red-400' }
    }
    if (nft.is_mint === 1) {
        return { label: 'Minting', className: 'border-blue-500/50 bg-blue-500/20 text-blue-400' }
    }
    return { label: 'Ready to Mint', className: 'border-green-500/50 bg-green-500/20 text-green-400' }
}
...
export function NFTLifecycleGrid() {
    const { address } = useAccount()
    const [nfts, setNfts] = useState<NFT[]>([])
    const [currentPage, setCurrentPage] = useState(1)
    const [totalPages, setTotalPages] = useState(1)
    const [filter, setFilter] = useState<ProfileFilter>('profile')
    const [counts, setCounts] = useState<NftStatusCounts>(EMPTY_COUNTS)
    ...
    const fetchNFTSnapshot = useCallback(async (showLoading = true) => {
        if (!address) {
            ...
            try {
                const apiUrl = `/api/user-nft-list?user_address=${address}&filter=${filter}&page=${currentPage}&page_size=20&include_chips=false`
    ...
    }, [isConfirmed, mintingNftId, address, associatedTransfer, fetchNFTSnapshot])
    ...
    }, [isBurnConfirmed, burningNftId, address, burnHash, fetchNFTSnapshot])
    ...
    return (
        <div>
            ...
            <div className="mb-6 mt-4 flex flex-wrap gap-2">
                <button
                    type="button"
                    onClick={() => { setFilter('profile'); setCurrentPage(1) }}
                    className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${filter === 'profile' ? 'border-blue-500/50 bg-blue-600/20 text-blue-400' : 'border-gray-700/50 bg-gray-800/50 text-gray-400'}`}
                >
                    All ({counts.profile})
                </button>
                <button
                    type="button"
                    onClick={() => { setFilter('mintable'); setCurrentPage(1) }}
                    className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${filter === 'mintable' ? 'border-blue-500/50 bg-blue-600/20 text-blue-400' : 'border-gray-700/50 bg-gray-800/50 text-gray-400'}`}
                >
                    Mintable ({counts.mintable})
                </button>
                <button
                    type="button"
                    onClick={() => { setFilter('minting'); setCurrentPage(1) }}
                    className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${filter === 'minting' ? 'border-blue-500/50 bg-blue-600/20 text-blue-400' : 'border-gray-700/50 bg-gray-800/50 text-gray-400'}`}
                >
                    Minting ({counts.minting})
                </button>
                <button
                    type="button"
                    onClick={() => { setFilter('burnable'); setCurrentPage(1) }}
                    className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${filter === 'burnable' ? 'border-blue-500/50 bg-blue-600/20 text-blue-400' : 'border-gray-700/50 bg-gray-800/50 text-gray-400'}`}
                >
                    Burnable ({counts.burnable})
                </button>
            </div>
            ...
            {isLoading ? (
                ...
                <>
                    ...
                    {nfts.map((nft) => {
                        ...
                        const status = getLifecycleStatus(nft)
                        ...
                        return (
                            ...
                            >
                                <div className="relative aspect-square bg-gray-800">
                                    <NFTProfileThumbnail
                                        nft={nft}
                                        onViewOriginal={() => setSelectedNft(nft)}
                                    />
                                    <span className={`absolute left-2 top-2 rounded-md border px-2 py-1 text-xs font-medium backdrop-blur-sm ${status.className}`}>
                                        {status.label}
                                    </span>
                                </div>
                            <div className="p-3">
